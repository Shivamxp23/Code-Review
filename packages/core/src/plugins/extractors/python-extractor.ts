...
export class PythonExtractor implements LanguageExtractor {
  ...
  extractStructure(rootNode: TreeSitterNode): StructuralAnalysis {
    ...
    const endpoints: StructuralAnalysis["endpoints"] = [];

    for (let i = 0; i < rootNode.childCount; i++) {
      const node = rootNode.child(i);
      if (!node) continue;

      // Detect FastAPI decorators on decorated_definition
      if (node.type === "decorated_definition") {
        for (let j = 0; j < node.childCount; j++) {
          const decorator = node.child(j);
          if (decorator && decorator.type === "decorator") {
            const decText = decorator.text;
            const routeMatch = decText.match(/@(app|router)\.(get|post|put|delete|patch|options|head)\s*\(\s*["']([^"']+)["']/);
            if (routeMatch) {
              endpoints.push({
    ...
    return { functions, classes, imports, exports, endpoints };
  }
