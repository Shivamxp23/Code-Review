...
import { validateGraph } from "@reviewer/core/schema";
...
import ArchitectureImpactView from "./components/ArchitectureImpactView";
import AtomicImpactView from "./components/AtomicImpactView";
import QADashboardView from "./components/QADashboardView";
import QAFailureGraph from "./components/QAFailureGraph";
...
function DashboardContent({
  ...
  const reviewerDetailLevel = useDashboardStore((s) => s.reviewerDetailLevel);
  const repositoryPickerOpen = useDashboardStore((s) => s.repositoryPickerOpen);
  const executionConfirmed = useDashboardStore((s) => s.executionConfirmed);
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-root text-text-primary noise-overlay">
          ...
          <div className="flex items-center bg-elevated rounded-lg p-0.5">
            <button
              ...
              className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${
                viewMode === "structural" && navigationLevel === "overview"
            ...
            <button
              ...
              className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${
                viewMode === "structural" && navigationLevel === "layer-detail"
                  ? "bg-accent/20 text-accent font-semibold"
                  : "text-text-muted hover:text-text-secondary"
              }`}
            >
      ...
      <div className="flex-1 flex min-h-0 relative">
          ...
          {viewMode === "review" ? (
            ...
            <div className="h-full flex flex-col overflow-auto p-4 gap-6">
              ...
              <div className="border-t border-border-subtle pt-4">
                <h3 className="text-sm font-semibold text-text-primary mb-3">Failure Drill-down</h3>
                <QAFailureGraph />
              </div>
            </div>
          ) : viewMode === "knowledge" ? (
        ...
        {/* Repository Picker Overlay */}
        {repositoryPickerOpen && <RepositoryPicker />}

        {/* Execution Confirmation Overlay */}
        {executionConfirmed && <ExecutionConfirmation />}
