...
import { SearchEngine } from "@reviewer/core/search";
import type { SearchResult } from "@reviewer/core/search";
import type { GraphIssue } from "@reviewer/core/schema";
import type {
  GraphNode,
  KnowledgeGraph,
  TourStep,
} from "@reviewer/core/types";
...
export type ViewMode = "structural" | "domain" | "knowledge" | "review" | "qa";
export type ReviewerMode = "human-friendly" | "agent-friendly";
export type ReviewerDetailLevel = "high-level" | "atomic";
...
interface DashboardStore {
  ...
  reviewerDetailLevel: ReviewerDetailLevel;
  setReviewerMode: (mode: ReviewerMode) => void;
  setReviewerDetailLevel: (level: ReviewerDetailLevel) => void;
  promptText: string;
  setPromptText: (text: string) => void;
  promptRequirements: any[];
  impactItems: any[];
  analysisStatus: "idle" | "analyzing" | "ready" | "error";
  analyzePrompt: () => void;
  implementationPlan: any | null;
  confirmExecution: () => void;
  dismissExecutionConfirmation: () => void;
  executionConfirmed: boolean;
  githubConnected: boolean;
  githubUser: { name: string; avatarUrl: string } | null;
  repositories: any[];
  selectedRepository: any | null;
  repositoryPickerOpen: boolean;
  setGithubConnected: (connected: boolean) => void;
  selectRepository: (repo: any) => void;
  toggleRepositoryPicker: () => void;
  qaReport: any | null;
  setQAReport: (report: any) => void;
...
export const useDashboardStore = create<DashboardStore>()((set, get) => ({
  ...
  // Reviewer & QA implementation
  reviewerMode: "human-friendly",
  reviewerDetailLevel: "high-level",
  setReviewerMode: (mode) => set({ reviewerMode: mode }),
  setReviewerDetailLevel: (level) => set({ reviewerDetailLevel: level }),
  promptText: "",
  setPromptText: (text) => set({ promptText: text }),
  promptRequirements: [
    { id: "req-1", title: "Add Reviewer Mode Toggle", type: "feature", priority: "high" },
    { id: "req-2", title: "Implement QA Skill Visualizer", type: "feature", priority: "high" },
    { id: "req-3", title: "Integrate GitHub Repo Switcher", type: "infrastructure", priority: "medium" }
  ],
  impactItems: [
    { nodeId: "file:packages/dashboard/src/App.tsx", path: "packages/dashboard/src/App.tsx", changeType: "modify", confidence: "high", reason: "Header navigation and view modes", estimatedScope: "moderate" },
    { nodeId: "file:packages/dashboard/src/store.ts", path: "packages/dashboard/src/store.ts", changeType: "modify", confidence: "high", reason: "Zustand state extension", estimatedScope: "minor" },
    { nodeId: "file:packages/dashboard/src/components/ReviewerModeSelector.tsx", path: "packages/dashboard/src/components/ReviewerModeSelector.tsx", changeType: "create", confidence: "high", reason: "New component", estimatedScope: "small" },
    { nodeId: "file:packages/dashboard/src/components/QADashboardView.tsx", path: "packages/dashboard/src/components/QADashboardView.tsx", changeType: "create", confidence: "high", reason: "New component", estimatedScope: "small" },
    { nodeId: "file:skills/reviewer-qa/SKILL.md", path: "skills/reviewer-qa/SKILL.md", changeType: "create", confidence: "high", reason: "QA Skill definition", estimatedScope: "minor" }
  ],
  analysisStatus: "ready",
  analyzePrompt: () => set({ analysisStatus: "analyzing" }),
  implementationPlan: {
    id: "plan-1",
    prompt: "Implement Reviewer Friendliness and QA Visualizer",
    requirements: [],
    impacts: [],
    tasks: [
      { id: "task-1", title: "Create Reviewer Mode UI", type: "create", files: ["ReviewerModeSelector.tsx"], symbols: ["ReviewerModeSelector"], dependencies: [], validation: ["npm test"], risks: [], estimatedScope: "small", status: "ready" },
      { id: "task-2", title: "Implement QA Dashboard", type: "create", files: ["QADashboardView.tsx"], symbols: ["QADashboardView"], dependencies: ["task-1"], validation: ["npm test"], risks: [], estimatedScope: "medium", status: "pending" }
    ],
    architectureImpact: "Extends existing dashboard views",
    testPlan: ["Unit test new components"],
    risks: ["None"],
    status: "ready",
    createdAt: new Date().toISOString()
  },
  confirmExecution: () => set({ executionConfirmed: true }),
  dismissExecutionConfirmation: () => set({ executionConfirmed: false }),
  executionConfirmed: false,
  githubConnected: true,
  githubUser: { name: "Reviewer User", avatarUrl: "https://github.com/github.png" },
  repositories: [
    { id: "repo-1", name: "Code-Review", owner: "user", fullName: "user/Code-Review", visibility: "public", defaultBranch: "main", language: "TypeScript", updatedAt: "2026-07-28" }
  ],
  selectedRepository: { id: "repo-1", name: "Code-Review", owner: "user", fullName: "user/Code-Review", visibility: "public", defaultBranch: "main", language: "TypeScript", updatedAt: "2026-07-28" },
  repositoryPickerOpen: false,
  setGithubConnected: (connected) => set({ githubConnected: connected }),
  selectRepository: (repo) => set({ selectedRepository: repo, repositoryPickerOpen: false }),
  toggleRepositoryPicker: () => set((state) => ({ repositoryPickerOpen: !state.repositoryPickerOpen })),
  qaReport: {
    id: "qa-report-1",
    repositoryName: "Code-Review",
    detectedStack: {
      platform: "Web",
    ...
    categories: [
      {
        ...
        status: "pass",
        results: [{ id: "b1", category: "Build", testName: "Vite Build", status: "pass", duration: 1200, affectedFiles: [], relatedNodes: [] }],
        summary: "Build succeeded cleanly"
      },
      {
        category: "Unit Tests",
        status: "pass",
        results: [{ id: "u1", category: "Unit", testName: "Store Navigation", status: "pass", duration: 400, affectedFiles: ["packages/dashboard/src/store.ts"], relatedNodes: [] }],
