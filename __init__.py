"""Hermes plugin for Reviewer (merged from Ponytail)."""
...
SKILL_COMMANDS = {
    "reviewer-lazy-review": "Review the current diff or provided target for over-engineering.",
    "reviewer-lazy-audit": "Audit the repo for over-engineering and deletion opportunities.",
    "reviewer-lazy-debt": "List every deliberate `reviewer:` shortcut and its upgrade path.",
    "reviewer-lazy-gain": "Show the measured-impact scoreboard (less code, less cost, more speed).",
    "reviewer-lazy-help": "Show the Reviewer lazy-mode command reference.",
}

ROOT = Path(__file__).resolve().parent
SKILLS_DIR = ROOT / "skills"
PONYTAIL_SKILL = SKILLS_DIR / "reviewer-lazy" / "SKILL.md"
REVIEW_SKILL = SKILLS_DIR / "reviewer-lazy-review" / "SKILL.md"
...
def _config_dir() -> Path:
    if os.environ.get("XDG_CONFIG_HOME"):
        return Path(os.environ["XDG_CONFIG_HOME"]) / "reviewer"
    if os.name == "nt":
        return Path(os.environ.get("APPDATA", Path.home() / "AppData" / "Roaming")) / "reviewer"
    return Path.home() / ".config" / "reviewer"


def _default_mode() -> str:
    env_mode = _normalize_config_mode(os.environ.get("REVIEWER_DEFAULT_MODE"))
    if env_mode:
        return env_mode
    try:
...
def _fallback_instructions(mode: str) -> str:
    return (
        f"REVIEWER LAZY MODE ACTIVE — level: {mode}\n\n"
...
def build_injected_context(mode: str | None = None) -> str:
    """Return the mode-filtered lazy-dev context injected before LLM turns."""
    ...
    if configured == "review":
        try:
            ...
            return f"REVIEWER LAZY MODE ACTIVE — level: review\n\n{_strip_frontmatter(body)}"
        except OSError:
            return "REVIEWER LAZY MODE ACTIVE — level: review. Review diffs for unnecessary complexity."

    effective = _normalize_runtime_mode(configured) or DEFAULT_MODE
    try:
        body = PONYTAIL_SKILL.read_text(encoding="utf-8")
        return f"REVIEWER LAZY MODE ACTIVE — level: {effective}\n\n{_filter_skill_body_for_mode(body, effective)}"
    except OSError:
...
def _skill_prompt(command: str, args: str = "") -> str:
    ...
    return (
        f"Load and follow the Hermes plugin skill `reviewer:{command}`. "
...
def rewrite_gateway_command(event: Any = None, gateway: Any = None, **_: Any) -> dict[str, str] | None:
    """Rewrite authorized gateway /reviewer-lazy-* commands into normal agent prompts."""
...
def _handle_mode_command(raw_args: str) -> str:
    ...
    if not arg:
        mode = _current_mode or _default_mode()
        return f"Reviewer lazy mode: {mode}. Use `/reviewer-lazy lite|full|ultra|off`."
    mode = _normalize_runtime_mode(arg)
    if not mode:
        return "Usage: /reviewer-lazy [lite|full|ultra|off]"
    _current_mode = mode
    return f"Reviewer lazy mode set to {mode}."
...
def register(ctx: Any) -> None:
    """Register Reviewer hooks, skills, and slash commands with Hermes."""
    ...
    ctx.register_command(
        ...
        description="Set Reviewer lazy senior dev mode: lite, full, ultra, or off.",
