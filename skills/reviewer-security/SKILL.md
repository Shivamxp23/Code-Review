...
**Language:** Parse `$ARGUMENTS` for a `lang=XX` parameter (e.g. `lang=de`, `lang=fr`). Default language is **English**. If a `lang` parameter is found, produce ALL output (epics, PDF report content, section headings, findings, summary) in the specified language. The `lang` parameter can be combined with a scope, e.g. `/reviewer-security docker lang=de`.
...
4. Create the output directory `docs/reviewer-security/` if it does not exist
5. Determine the output filename. Use the timestamp format `YYYY-MM-DD-HHmmSS` (current date and time):
   - **First audit** (no existing PDF in `docs/reviewer-security/`): `[projectname]-security-audit-YYYY-MM-DD.pdf`
...
6. Write the populated HTML file to `docs/reviewer-security/[filename].html`
7. Detect the platform and resolve the Chrome binary path:
   ...
   <resolved-chrome-path> \
     ...
     --print-to-pdf="docs/reviewer-security/[filename].pdf" \
     --print-to-pdf-no-header \
     "docs/reviewer-security/[filename].html"
   ```
...
- docs/reviewer-security/[projectname]-security-audit-YYYY-MM-DD.pdf (first audit)
- docs/reviewer-security/[projectname]-security-review-YYYY-MM-DD-HHmmSS.pdf (subsequent reviews)
```
