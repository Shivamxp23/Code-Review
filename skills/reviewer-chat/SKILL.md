...
Answer questions about this codebase using the knowledge graph in the project's data directory (`.reviewer-data/knowledge-graph.json`, or the legacy `.reviewer/knowledge-graph.json` when that directory is present).
...
1. **Resolve the data directory `$UA_DIR`.** Run `UA_DIR=$([ -d .reviewer ] && echo .reviewer || echo .ua)` — this is the legacy `.reviewer/` when it already exists, otherwise the new `.reviewer-data/`. Check that `$UA_DIR/knowledge-graph.json` exists in the current project root. If not, tell the user to run `/reviewer` first.
...
2. **Check graph freshness before using graph-derived context**:
   ...
   - If the committed diff or any working-tree command reports project files, warn before answering that graph-derived context may omit those changes. Suggest: Run `/reviewer` to refresh the graph.
