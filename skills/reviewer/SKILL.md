...
Analyze the current codebase and produce a `knowledge-graph.json` file in the project's data directory (`.reviewer-data/`, or the legacy `.reviewer/` when it already exists). This file powers the interactive dashboard for exploring the project's architecture.
...
1.5. **Ensure the plugin is built.** Later phases invoke Node scripts that import `@Reviewer/core`. On a fresh install `packages/core/dist/` does not exist yet — build once.

   **Important:** do **not** assume the plugin root is simply two directories above the skill path string. In many installations `~/.agents/skills/reviewer` is a symlink into the real plugin checkout. Prefer runtime-provided plugin roots first (for Claude), then fall back to universal symlinks, skill symlink resolution, and common clone-based install paths.

   Resolve the plugin root like this:

   ```bash
   SKILL_REAL=$(realpath ~/.agents/skills/reviewer 2>/dev/null || readlink -f ~/.agents/skills/reviewer 2>/dev/null || echo "")
   SELF_RELATIVE=$([ -n "$SKILL_REAL" ] && cd "$SKILL_REAL/../.." 2>/dev/null && pwd || echo "")
   COPILOT_SKILL_REAL=$(realpath ~/.copilot/skills/reviewer 2>/dev/null || readlink -f ~/.copilot/skills/reviewer 2>/dev/null || echo "")
   COPILOT_SELF_RELATIVE=$([ -n "$COPILOT_SKILL_REAL" ] && cd "$COPILOT_SKILL_REAL/../.." 2>/dev/null && pwd || echo "")

   PLUGIN_ROOT=""
   for candidate in \
     "${CLAUDE_PLUGIN_ROOT}" \
     "$HOME/.reviewer-plugin" \
     "$SELF_RELATIVE" \
     "$COPILOT_SELF_RELATIVE" \
     "$HOME/.codex/reviewer-anything/reviewer-anything-plugin" \
     "$HOME/.opencode/reviewer-anything/reviewer-anything-plugin" \
     "$HOME/.pi/reviewer-anything/reviewer-anything-plugin" \
     "$HOME/reviewer-anything/reviewer-anything-plugin"; do
   ...
   if [ -z "$PLUGIN_ROOT" ]; then
     echo "Error: Cannot find the Reviewer plugin root."
     echo "Checked:"
     echo "  - ${CLAUDE_PLUGIN_ROOT:-<unset CLAUDE_PLUGIN_ROOT>}"
     echo "  - $HOME/.reviewer-plugin"
     echo "  - ${SELF_RELATIVE:-<unresolved path derived from ~/.agents/skills/reviewer>}"
     echo "  - ${COPILOT_SELF_RELATIVE:-<unresolved path derived from ~/.copilot/skills/reviewer>}"
     echo "  - $HOME/.codex/reviewer-anything/reviewer-anything-plugin"
     echo "  - $HOME/.opencode/reviewer-anything/reviewer-anything-plugin"
     echo "  - $HOME/.pi/reviewer-anything/reviewer-anything-plugin"
     echo "  - $HOME/reviewer-anything/reviewer-anything-plugin"
     echo "Make sure the plugin is installed correctly."
     exit 1
   fi

   if [ ! -f "$PLUGIN_ROOT/packages/core/dist/index.js" ]; then
     cd "$PLUGIN_ROOT" && (pnpm install --frozen-lockfile 2>/dev/null || pnpm install) && pnpm --filter @Reviewer/core build
   fi
   ```

   If `pnpm` is missing, report to the user: "Install Node.js ≥ 22 and pnpm ≥ 10, then re-run `/reviewer`."

1.7. **Resolve the data directory `$UA_DIR`.** All Reviewer artifacts live in the project's data directory. Resolve it once, now that `$PROJECT_ROOT` is known, and reuse `$UA_DIR` for every read and write in later phases:
   ```bash
   UA_DIR="$PROJECT_ROOT/$([ -d "$PROJECT_ROOT/.reviewer" ] && echo .reviewer || echo .ua)"
   ```
   This keeps the legacy `.reviewer/` directory when it already exists (existing projects keep working with no migration) and uses the new `.reviewer-data/` otherwise. Because each phase may run in a fresh shell, treat `$UA_DIR` — like `$PROJECT_ROOT` — as a value you carry forward and substitute; re-resolve it with the line above if a later command block needs it in a new shell.
...
2. **If it does NOT exist**, generate a starter file by invoking the bundled script (delegates to `generateStarterIgnoreFile` in `@Reviewer/core`, which reads `.gitignore`, deduplicates against built-in defaults, and emits language-grouped test-file suggestions). Pass `$PLUGIN_ROOT` via the env so the script doesn't have to re-derive it from its own path (which breaks for copied skill installs):
     ```bash
...
6. Only automatically launch the dashboard by invoking the `/reviewer-dashboard` skill if final graph validation passed after normalization/review fixes.
