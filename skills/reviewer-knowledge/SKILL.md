...
1. Determine the target directory:
   ...
   - **Resolve the data directory `$UA_DIR`** once, and reuse it for every read and write below: `UA_DIR="<TARGET_DIR>/$([ -d "<TARGET_DIR>/.reviewer" ] && echo .reviewer || echo .ua)"` — this selects the legacy `.reviewer/` when it already exists, otherwise the new `.reviewer-data/`.
...
5. Clean up intermediate files. Resolve `$UA_DIR` into a shell variable and guard it so an empty or unresolved path can never expand to `rm -rf /intermediate` (deleting from the filesystem root):
   ...
   UA_DIR="$TARGET_DIR/$([ -d "$TARGET_DIR/.reviewer" ] && echo .reviewer || echo .ua)"
