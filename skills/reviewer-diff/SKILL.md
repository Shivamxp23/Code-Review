...
Analyze the current code changes against the knowledge graph in the project's data directory (`.reviewer-data/knowledge-graph.json`, or the legacy `.reviewer/knowledge-graph.json` when that directory is present).
...
1. **Resolve the data directory `$UA_DIR`.** Run `UA_DIR=$([ -d .reviewer ] && echo .reviewer || echo .ua)` — this is the legacy `.reviewer/` when it already exists, otherwise the new `.reviewer-data/`. Check that `$UA_DIR/knowledge-graph.json` exists. If not, tell the user to run `/reviewer` first.
...
3. **Read project metadata and check graph freshness** — use Grep or Read with a line limit to extract the `"project"` section, including `gitCommitHash` as `GRAPH_COMMIT_RAW`, then:
   ...
   - If the committed diff or any working-tree command reports project files, warn before impact analysis that the graph may omit those changes. Suggest: Run `/reviewer` to refresh the graph.
...
8. **Write diff overlay for dashboard** — after producing the analysis, write the diff data to `$UA_DIR/diff-overlay.json` so the dashboard can visualize changed and affected components. The file contains:
   ...
   ```
   After writing, tell the user they can run `/reviewer-anything:understand-dashboard` to see the diff overlay visually.
