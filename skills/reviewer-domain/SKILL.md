...
description: Extract business domain knowledge from a codebase and generate an interactive domain flow graph. Works standalone (lightweight scan) or derives from an existing /reviewer knowledge graph.
argument-hint: "[--full]"
---
...
- If a knowledge graph already exists (`.reviewer-data/knowledge-graph.json`, or the legacy `.reviewer/knowledge-graph.json` when that directory is present), derives domain knowledge from it (cheap, no file scanning)
...
**Resolve the data directory `$UA_DIR`.** All Reviewer artifacts live in the project's data directory. Resolve it once, now that `$PROJECT_ROOT` is known, and reuse `$UA_DIR` for every read and write in later phases:
```bash
UA_DIR="$PROJECT_ROOT/$([ -d "$PROJECT_ROOT/.reviewer" ] && echo .reviewer || echo .ua)"
```
This keeps the legacy `.reviewer/` directory when it already exists (existing projects keep working with no migration) and uses the new `.reviewer-data/` otherwise. Because each phase may run in a fresh shell, carry `$UA_DIR` forward like `$PROJECT_ROOT`, re-resolving it with the line above if a later command block needs it.

**Important:** do **not** assume the plugin root is simply two directories above the skill path string. In many installations `~/.agents/skills/reviewer-domain` is a symlink into the real plugin checkout. Prefer runtime-provided plugin roots first (for Claude), then fall back to universal symlinks, skill symlink resolution, and common clone-based install paths.

Resolve the plugin root like this:

```bash
SKILL_REAL=$(realpath ~/.agents/skills/reviewer-domain 2>/dev/null || readlink -f ~/.agents/skills/reviewer-domain 2>/dev/null || echo "")
SELF_RELATIVE=$([ -n "$SKILL_REAL" ] && cd "$SKILL_REAL/../.." 2>/dev/null && pwd || echo "")
COPILOT_SKILL_REAL=$(realpath ~/.copilot/skills/reviewer-domain 2>/dev/null || readlink -f ~/.copilot/skills/reviewer-domain 2>/dev/null || echo "")
...
if [ -z "$PLUGIN_ROOT" ]; then
  ...
  echo "  - ${SELF_RELATIVE:-<unresolved path derived from ~/.agents/skills/reviewer-domain>}"
  echo "  - ${COPILOT_SELF_RELATIVE:-<unresolved path derived from ~/.copilot/skills/reviewer-domain>}"
...
2. If it exists AND `--full` was NOT passed, check freshness before deriving from it:
   ...
   - Ignore the selected data directory (`.reviewer-data/` or legacy `.reviewer/`) in every command's output because it contains generated graph artifacts, not project source drift.
   - If the committed diff or any working-tree command reports project files, warn that domain extraction may omit those changes. Suggest: Run `/reviewer` to refresh the knowledge graph.
...
1. Auto-trigger `/reviewer-dashboard` to visualize the domain graph
